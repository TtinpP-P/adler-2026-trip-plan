...
export function buildOfflinePlanHtml() {
  ...
  *{box-sizing:border-box}html{scroll-behavior:smooth}body{max-width:1080px;margin:auto;padding:28px 18px 72px;background:var(--bg);color:var(--text);font:14px/1.55 system-ui,-apple-system,sans-serif}
  ...
  @media(max-width:700px){html,body{width:100%;max-width:100%;overflow-x:clip;-webkit-text-size-adjust:100%;text-size-adjust:100%}body{min-height:100dvh;padding:max(18px,env(safe-area-inset-top,0px)) 14px max(48px,calc(env(safe-area-inset-bottom,0px) + 28px));font-size:15px}.hero{padding:26px 0 18px}.hero p{font-size:14px}nav{top:env(safe-area-inset-top,0px);gap:8px;margin-inline:-14px;padding:9px 14px}nav a,.links a{min-height:44px;padding:9px 11px;touch-action:manipulation}.day-grid,.grid{grid-template-columns:1fr}.day{padding:22px 0}.day-head{display:block}.day-head>strong{display:block;margin-bottom:14px}.timeline li{grid-template-columns:50px minmax(0,1fr);gap:8px;padding:14px 0}.timeline p,.meal p,.card p{font-size:14px;line-height:1.55}.meal,.card{padding:14px}.links{gap:8px}.table-wrap{overflow:visible}table,thead,tbody,tr,th,td{display:block;width:100%;min-width:0}thead{display:none}tbody{display:grid;gap:9px}tr{padding:10px 12px;background:var(--panel);border:1px solid var(--line)}td,td:last-child{padding:5px 0;border:0;text-align:left;white-space:normal}td::before{display:block;color:var(--muted);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}td:nth-child(1)::before{content:"Статья"}td:nth-child(2)::before{content:"Расчёт"}td:nth-child(3)::before{content:"Комментарий"}td:nth-child(4)::before{content:"Сумма"}.total{font-size:16px}}
  @media(max-width:400px){body{padding-right:12px;padding-left:12px}nav{margin-inline:-12px;padding-inline:12px}h1{font-size:38px}.timeline li{grid-template-columns:46px minmax(0,1fr)}}
  @media(max-width:900px) and (orientation:landscape) and (max-height:520px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.day-grid{grid-template-columns:1.1fr .9fr}}
